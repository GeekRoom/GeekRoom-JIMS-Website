  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-6 py-12 border-t border-white/10 relative">
      <div className="mb-8">
        ...
        <h2 className="font-display text-3xl sm:text-4xl text-white font-bold tracking-tight mt-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {purposes.map((p) => (
          <div
            key={p.id}
            className={`purpose-card-interactive ${activePurpose === p.id ? 'active' : ''}`}
            onClick={() => setActivePurpose(activePurpose === p.id ? null : p.id)}
          >
